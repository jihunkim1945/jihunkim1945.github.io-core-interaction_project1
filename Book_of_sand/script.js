...
const TV_IMAGES = [
  ...
  "../assets/images/Subject2.png",
  "../assets/images/Subject3.png",
  "../assets/images/subject4.JPG"
];

const STATIC_GIF = "../assets/images/analog.gif";
...
TEXT (6문단)
**********************/
const FULL_TEXT = `. . . and I hung up the phone. Immediately I recollected the voice that had spoken in 
...
문단 분할
**********************/
function splitIntoChunks(text, n=6){
...
function wrapLines(text, size=70){
  const words = text.split(" ");
  let line="";
  const lines=[];
  for(let w of words){
    if((line+w).length > size){
      lines.push(line);
      line=w+" ";
    }else{
      line+=w+" ";
...
function startGlitch(el){
  ...
  el._t = setInterval(()=>{
    el.textContent = original.split("").map(ch=>{
      if(ch===" ") return " ";
      return Math.random()<0.3 ? ch : GLITCH_CHARS[Math.floor(Math.random()*GLITCH_CHARS.length)];
    }).join("");
  },40);
}
...
function staticBlast(){
  overlay.style.backgroundImage = `url(${STATIC_GIF})`;
  overlay.classList.add("blast");
  setTimeout(()=>overlay.classList.remove("blast"),500);
}
...
**********************/
function placeTV(){
  const tv = document.getElementById("tvButton");
  tv.style.left = rand(5,85)+"vw";
  tv.style.top  = rand(5,85)+"vh";
}

function setTVImage(){
  document.getElementById("tvBtnImg").src = pick(TV_IMAGES);
}
...
SAND EFFECT
**********************/
function sandify(clicked){
  const paras = document.querySelectorAll(".para");
  const sand = document.getElementById("sandLayer");
  ...
  paras.forEach(p=>{
    ...
    for(let ch of text){
      ...
      span.animate([
        ...
        {transform:`translateY(${rand(200,600)}px)`}
      ],{
        duration:rand(800,1500),
        easing:"ease-in",
        fill:"forwards"
      }).onfinish=()=>{
        sand.appendChild(span);
      };
    }
  });
}
...
function buildText(){
  ...
  const chunks = splitIntoChunks(FULL_TEXT,6);

  chunks.forEach(chunk=>{
    const p = document.createElement("div");
    p.className="para";

    const lines = wrapLines(chunk,70);

    lines.forEach(t=>{
      const line = document.createElement("span");
      line.className="line";
      line.textContent=t;
      line.dataset.original=t;

      line.addEventListener("mouseenter",()=>startGlitch(line));
      line.addEventListener("mouseleave",()=>stopGlitch(line));

      p.appendChild(line);
    });

    p.style.left = rand(-10,70)+"vw";
    p.style.top  = rand(-10,80)+"vh";
...
**********************/
function init(){
  document.body.style.background = pick(BG_COLORS);
